...
Assuming that the generator $\theta$ of the torsion group $W=\langle{\theta}\rangle$ of $k$ is embedded in $\mathbb{C}\times\mathbb{C}$ as 

$$\theta=(\zeta_m^{\ell},\zeta_m),\quad \text{ where } \ell=3 \text{ if } m=8,10, \text{ and } \ell=5 \text{ if } m=12.$$

Here we computationally verify the following claims:


-   $P^{\Delta,\Delta}(c')\subset S\subset P^{\Delta,\Delta}(d'),$ for certain constants $c'>0$ and $d'>0$.
-    $E\cdot S\subset S$
...
- $P^{\Delta,\Delta}(\lambda)$ denotes the complex of polyhedral cones defined in 
[Attractor-Repeller...](https://www.sciencedirect.com/science/article/pii/S0022314X23002299)

$$P^{\Delta,\Delta}(\lambda)=\bigcup_{i=1}^3\mathbb{R}_{\geq0}\cdot (\Delta\times (\lambda^{-1}\partial\Delta))$$ 

- $S$ denotes the complex of polyhedral cones with coefficients in the field $k$

$$S:=\displaystyle\bigcup_{j=0}^{m-1} \mathcal{C}_j,\qquad $$