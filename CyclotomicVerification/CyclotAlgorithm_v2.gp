...
\\ Claim I:
\\       S=\bicup_{j=0}^{m-1}w^{j}*C_0 (union of cones) is a polyhedral complex of "m" pointed cones,for m=8, 10 or 12.
\\and S can be write as a (disjoint) union: {0}\cup (\bigcup w^{j}*C'_0)={0}\cup S' for j=0,1,...,m-1, 
...
\\    S is a polyhedral complex of "m" pointed polyhdral cones, for m=8, 10 or 12.
\\    and S can be write as a disjoint union: {0}\cup (\bigcup w^{j}*C'_0) for j=0,1,...,m-1


\\STEP 1:
\\ Input: given "bnf=bnfinit(p)", where p = x^4+1, x^4-x^3+x^2-x+1 or x^4-x^2+1 
\\ Output: return the list of intersection L0:=[C_0\cap C_j for j=1,2,...,m-1]
\\ and L1:=C0-(S'-{0}),
\\ since we want to verify that S (union of closed polyhedral cones) can be write as 
\\ a disjoint union {0}\cup S':= {0}\cup (\bigcup w^{j}*C'_0) for j=0,1,...,m-1 (of semi-closed cones)
\\ It is clear that {0}\cup S' is contained in S
\\ Only need to verify that S is contained in {0}\cup S'
\\ for this, it is enough to compute the difference: C_0-S' (one cone with the complex S')
\\ we want to verify that L1=C_0-(S') is [] which it is interpreted in this case as the vertex {0}

ListInterDifConeS(bnf)=
{my(k,p,m,w,h1,h2,l,S,S1,L0,L1,C1,v2);
k=bnf; p=k.pol;
...
S=vector(m,j,lift(Mod(w^(j-1)*[1,w,w^2,w^3],p))); \\The union S of the "m" cones given by generators
S1=vector(m,j,concat(HV(S[j],k),[[1,1,1,1]])); \\ now given by [generators, inequalities, [1,1,1,1]]
L0=vector(m-1,j);
for(j=2,m, v2=intCones2([S1[1],S1[j]],k);L0[j-1]=v2[1][1]);
C1=kcomplex2(S,k);  \\ represent the complex: S' of semi-closed cones
L1=[S1[1]];
for(i=1,#C1, if(#L1>0, L1=kdcomplex(L1,C1[i],k))); \\difference of closed cone C_0:=S1[1] with the complex C1=S'
return([L0,L1[1][1]]);  \\[[list of intersections C0\cap C_j], C0-C1]
}
