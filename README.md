...
* $v =$ vector of four entries [#(four-cones),#(three-cones),#(two-cones),#(one-cones)] which describes information of the number semi-closed j-cones (by dimension $j=1,2,3,4$) obtained in a Shintani domain obtained by execution of command $torFDK41(p,flag)$.
