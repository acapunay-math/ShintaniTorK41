...
* $r' =$ is a positive integer $\leq r$ such that $E^{r'}\cdot S\subset S$, where $S$ is a polyhedral complex of cones is described in MS.

* $N =$ number of (semi-closed) four-cones in the Shintani domain constructed 

* $v =$ vector of four entries which describing [#(four-cones),#(three-cones),#(two-cones),#(one-cones)]


The Shintani domain corresponds to the action on $(\mathbb{C}^{\ast})\times(\mathbb{C}^{\ast})$ of the group generated by $E$
